...
const VerticalCardProduct = ({ category, heading }) => {
    ...
    return (
        <div className='container mx-auto px-4 my-6 relative'>
            ...
            <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 items-center gap-4 md:gap-6 overflow-x-auto scrollbar-none transition-all' ref={scrollElement}>
                ...
                {loading ? (
                    loadingList.map((product, index) => (
                        <div key={index} className='min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white rounded-sm shadow'>
                    ...
                    data.map((product, index) => (
                        <Link to={"product/" + product?._id} key={product?._id} className=' bg-white text-center rounded-sm shadow'>
