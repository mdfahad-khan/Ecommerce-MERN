...
const VerticalCardProduct = ({ category, heading }) => {
  ...
  return (
    <div className="container mx-auto px-4 my-6 relative">
      ...
      <div
        className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 items-center gap-4 md:gap-6 overflow-x-auto scrollbar-none transition-all"
        ref={scrollElement}
      >
        <button
        ...
        {loading
          ...
          : data.map((product, index) => (
              <Link
                ...
                key={product?._id}
                className=" bg-white text-center rounded-sm shadow"
              >
                ...
                <div className="p-4 grid gap-3 ">
                  ...
                  <div className="flex gap-2">
                    ...
                    <p className="text-red-700 line-through text-sm ">
                      {product?.price} TK
                    </p>
                  </div>
                  <button
                    className="text-sm bg-[#192A56] hover:bg-[#16a085] text-white px-3 py-0.5 rounded-sm"
